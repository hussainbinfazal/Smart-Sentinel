...
const Whatwedo = () => {
  ...
  return (
    <div className="max-w-screen-2xl w-full min-h-[300px] bg-white dark:bg-black/40 flex flex-col items-center justify-center p-4 gap-4">
      <h2 className="uppercase text-sm">What We Do </h2>
      <h2 className="uppercase text-4xl text-center">
        <span className="text-green-800 font-semibold">Smart Sentinel</span> is
        for your{" "}
      </h2>
      <div className="flex flex-col md:flex-row gap-4 w-full min-h-[400px] justify-center items-center md:items-start mt-8">
        {cards.map((card) => (
          <Card className="w-full max-w-sm md:max-w-full min-h-[380px] max-h-[500px] overflow-hidden dark:bg-black/40">
            <CardContent className="flex flex-col gap-7 pb-4">
              <span className="text-black/80 dark:text-white w-full justify-center flex  text-lg">
                {card?.icon}
              </span>
              <h2 className="text-2xl font-semibold text-center">{card?.heading}</h2>
              <p className="text-black/110 text-center">{card?.text}</p>
            </CardContent>
